...
// Must start with a letter or underscore, contain '=', and the part before '='
...
func isRedirect(s string) bool {
	// Patterns: >, >>, <, <<, 2>, 2>>, &>, &>>, 2>&1, 1>&2, etc.
	// Also bare tokens like ">file" or "2>file".
...
// hasRedirectTarget checks if a redirect token already includes its target.
// e.g., ">file" has a target, but ">" alone does not.
func hasRedirectTarget(s string) bool {
	// Strip the operator part, see if anything remains.
	trimmed := strings.TrimLeft(s, "0123456789")
